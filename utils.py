...
def parse_args():
    ...
    ## Run details
    parse.add_argument("--model", help="model to train and test", type=str, default="PPR-KNN-GCNH")
    ...
    # parse.add_argument('--k', type=int, default=10, help='Number of nearest neighbors for KNN graph')
    # parse.add_argument('--ppr_threshold', type=float, default=0.1, help='Threshold for sparsifying PPR matrix')
...
def normalize(adj, is_sparse=False):
    ...
    if is_sparse:
        ...
        if values.is_cuda:
            # Pure PyTorch path for CUDA sparse tensors (D^{-1/2} A D^{-1/2})
            n = adj.shape[0]
            ...
            return torch.sparse_coo_tensor(indices, normalized_values, adj.shape, device=values.device).coalesce()
...
def sparsify_ppr(ppr_matrix, threshold=0.1):
    """
    对PPR扩散矩阵进行稀疏化
    Args:
        ppr_matrix: PPR扩散矩阵 (torch.Tensor)
        threshold: 阈值，小于该值的元素将被置为0
    Returns:
        sparse_ppr: 稀疏化后的PPR矩阵
    """
    # 创建掩码
    mask = ppr_matrix > threshold
    # 应用掩码并重新归一化
    sparse_ppr = ppr_matrix * mask.float()
    # 重新归一化
    row_sum = sparse_ppr.sum(dim=1)
    row_sum[row_sum == 0] = 1  # 避免除零
    sparse_ppr = sparse_ppr / row_sum.unsqueeze(1)
    return sparse_ppr

def compute_ppr(adj, alpha, threshold):
    """
    计算Personalized PageRank扩散矩阵并进行稀疏化
    使用矩阵求逆的方式计算PPR
    Args:
        adj: 邻接矩阵 (torch.Tensor)
        alpha: 重启概率
        threshold: 稀疏化阈值
    Returns:
        ppr_matrix: 稀疏化后的PPR扩散矩阵
    """
    n = adj.shape[0]
    
    # 添加自环
    adj_loop = adj + torch.eye(n, device=adj.device)
    
    # 计算对称转移矩阵
    deg_loop = adj_loop.sum(dim=1)
    deg_loop_invsqrt = 1 / torch.sqrt(deg_loop)
    deg_loop_invsqrt = torch.diag(deg_loop_invsqrt)
    
    # T_sym = D^(-1/2) * A * D^(-1/2)
    T_sym = torch.matmul(torch.matmul(deg_loop_invsqrt, adj_loop), deg_loop_invsqrt)
    
    # PPR-based diffusion: S = α * (I - (1-α)*T_sym)^(-1)
    I = torch.eye(n, device=adj.device)
    ppr_matrix = alpha * torch.inverse(I - (1 - alpha) * T_sym)
    
    # 稀疏化
    ppr_matrix = sparsify_ppr(ppr_matrix, threshold)
