"""
Perform training and testing of Hybrid-GCNH on the synthetic dataset
"""
...
from models_hybrid import Hybrid_GCNH  # 改用Hybrid_GCNH模型
...
if __name__ == "__main__":
    ...
    # 创建结果保存目录
    save_dir = 'd:\\Project\\CMA-GNN\\experiment_results'
    ...
    for hom_syn in ["h0.00-r", "h0.10-r", "h0.20-r", "h0.30-r", "h0.40-r", "h0.50-r", "h0.60-r", "h0.70-r", "h0.80-r", "h0.90-r", "h1.00-r"]:
        ...
        print(f"Homophily {hom_syn}: Accuracy {np.mean(final_acc):.4f} ± {np.std(final_acc):.4f}")

    # 在开始实验前，先记录当前时间和模型名称
    from datetime import datetime
    current_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    model_name = "Hybrid-GCNH"  # 可以根据需要修改模型名称

    # 将结果保存到文件（使用追加模式'a'）
    with open(os.path.join(save_dir, 'synthetic_results.txt'), 'a') as f:
        # 添加分隔线和实验信息
        f.write("\n" + "="*50 + "\n")
        ...
        # 写入表头
        f.write("Homophily\tAccuracy\tStd\n")
        
        # 写入结果
        for hom in all_results:
            f.write(f"{hom}\t{all_results[hom]['mean']:.4f}\t{all_results[hom]['std']:.4f}\n")
